...
interface AdoptionDropdownProps {
    isMobile: boolean;
}

const AdoptionDropdown: React.FC<AdoptionDropdownProps> = ({ isMobile }) => {
    const location = useLocation();
    const isMainPage = location.pathname === '/';

    const menuItems = [
        { to: "/adoption", text: "입양 공고" },
        { to: "/adoption/guide", text: "입양 방법" },
    ];

    if (isMobile) {
        return (
            <div className="py-2">
                <span className="block text-[18px] font-medium mb-2">입양</span>
                {menuItems.map((item) => (
                    <Link
                        key={item.to}
                        to={item.to}
